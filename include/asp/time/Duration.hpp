    ...
    public:
        ...
        ASP_CLANG_CONSTEXPR static std::optional<Duration> fromSecsF32(f32 secs) {
            ...
            return Duration(
                ...
                static_cast<u32>(static_cast<f32>(time_detail::NANOS_IN_SEC) * fractional)
            );
        }
        ...
        ASP_CLANG_CONSTEXPR static std::optional<Duration> fromSecsF64(f64 secs) {
            ...
            return Duration(
                static_cast<u64>(whole),
                static_cast<u32>(static_cast<f64>(time_detail::NANOS_IN_SEC) * fractional)
            );
        }
        ...
        constexpr static inline Duration fromMinutes(u64 mins) { return _unchecked(time_detail::minutes_to_secs(mins), 0); }
        constexpr static inline Duration fromHours(u64 hours) { return _unchecked(time_detail::hours_to_secs(hours), 0); }
        constexpr static inline Duration fromDays(u64 days) { return _unchecked(time_detail::days_to_secs(days), 0); }
        constexpr static inline Duration fromWeeks(u64 weeks) { return _unchecked(time_detail::weeks_to_secs(weeks), 0); }
        constexpr static inline Duration fromYears(u64 years) { return _unchecked(time_detail::years_to_secs(years), 0); }
        ...
        constexpr inline u64 minutes() const { return time_detail::secs_to_minutes(m_seconds); }
        constexpr inline u64 hours() const { return time_detail::secs_to_hours(m_seconds); }
        constexpr inline u64 days() const { return time_detail::secs_to_days(m_seconds); }
        constexpr inline u64 weeks() const { return time_detail::secs_to_weeks(m_seconds); }

        constexpr inline u64 subsecMillis() const { return time_detail::nanos_to_millis(m_nanos); }
